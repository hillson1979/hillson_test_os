...
typedef struct module {
  ...
  uint32_t state;
  uint32_t padding[2];    /* Ensure size is a multiple of pointer size. */
} module_t;
...
/* Perform a backtrace.
   ...
   is complete. */
uint32_t backtrace(uint32_t *data, struct regs *regs);
...
int set_insn_breakpoint(uint32_t loc);
...
int set_read_breakpoint(uint32_t loc);
...
int set_write_breakpoint(uint32_t loc);
...
const char *lookup_kernel_symbol(uint32_t addr, int *offs);
...
uint32_t round_to_page_size(uint32_t x);
...
uint64_t alloc_pages(int req, unsigned int num);
int free_pages(uint64_t pages, unsigned int num);
...
bool cow_handle_page_fault(uint32_t addr, uint32_t error_code);
