...
static struct ohci_ed_t *alloc_ed(struct ohci_dev_t *ohci)
{
    ...
    usb_printk("%s: failed to alloc ED\n", "ohci");
...
static struct ohci_ed_t *ed_from_phys(struct ohci_dev_t *ohci, uintptr_t phys)
{
    ...
    usb_printk("%s: failed to find ED: " _XPTR_ "\n", "ohci", phys);
...
static void check_port_status(volatile struct ohci_dev_t *ohci, unsigned int port)
{
    ...
    usb_printk("ohci: port %u: %sspeed device %s\n", 
            port,
...
static void ohci_schedule_transfer(struct usb_transfer_t *transfer)
{
    ...
    if(!ed)
    {
        usb_printk("%s: transfer with NULL endpoint descriptor (in %s)\n", "ohci", __func__);
        return;
    }
...
static void ohci_wait_transfer(struct usb_transfer_t *transfer)
{
    ...
    while(timeout--)
    {
        ...
        //usb_printk("ohci_wait_transfer: td->dword0 0x%x, timeout %d\n", td->dword0, timeout);
    ...
    for(usbtrans = transfer->trans_head; usbtrans != NULL; usbtrans = usbtrans->next)
    {
        ...
        //usb_printk("ohci_wait_transfer: usbtrans 0x%lx, success %d\n", usbtrans, transfer->success);
                            ...
                            uint8_t direction)
{
    ...
    if(transfer->type == USB_TRANSFER_INTERRUPT)
    {
        ...
        {
            usb_printk("%s: failed to set up transaction descriptor copy\n", "ohci");
            return -ENOMEM;
        }
    }
...
int ohci_install(pci_compat_t *pci, struct pci_bar_t *bar)
{
    ...
    if(dword & OHCI_CTRL_INTREDIR)
    {
        ...
        if(i < 100)
        {
            usb_printk("ohci: controller ownership changed successfully\n");
        }
        else
        {
            usb_printk("ohci: could not change controller ownership\n");
    ...
    else
    {
        ...
        {
            usb_printk("ohci: active BIOS driver present\n");
...
#define ALLOC_MEMPAGE(s, p, v) \
    ...
    usb_printk("%s: hcca virt " _XPTR_ ", phys " _XPTR_ "\n", 
    ...
    usb_printk("%s: edpool virt " _XPTR_ ", phys " _XPTR_ "\n", 
...
int ohci_intr(struct regs *r, void *arg)
{
    ...
    if(!ohci)
    {
        usb_printk("ohci_intr: not from here\n");
        return 0;
    }
    ...
    if(!dword)
    {
        ...
        //usb_printk("ohci_intr: dword 0\n");
        return 0;
    }

    //usb_printk("ohci_intr: dword 0x%x\n", dword);
