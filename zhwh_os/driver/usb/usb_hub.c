...
kernel_mutex_t usb_hub_tablock;
...
static int hub_get_descriptor(struct usb_hub_t *hub)
{
    ...
    if(!res)
    {
        usb_printk("usb-hub: failed to get hub %s\n", "descriptor");
    }
...
static int hub_port_connected(struct usb_hub_t *hub, uint8_t port)
{
    ...
    usb_printk("usb-hub: failed to get port %d %s\n", port, "connection status");
...
static int hub_setup_device(volatile struct usb_hub_t *hub, unsigned int port, uint8_t speed)
{
    ...
    {
        usb_printk("usb-hub: failed to create USB device\n");
        return -ENOMEM;
    }
    ...
    {
        usb_printk("usb-hub: failed to set up USB device\n");
                            ...
                            uint8_t sel, uint8_t port)
{
    ...
    if(!res)
    {
        usb_printk("usb-hub: failed to %s feature %d on port %d\n", "set", feature, port);
    }
...
void usb_hub_remove_dev(struct usb_dev_t *usb)
{
    ...
    if(!hub)
    {
        usb_printk("usb-hub: cannot find USB hub in list\n");
        return;
    }
        ...
        for(i = 1; i <= hub->desc.ports; i++)
        {
            ...
            if(portst.state_change)
            {
                ...
                if(portst.curstat && 
                   !(hub->ports[i].flags & HUB_PORT_FLAG_CONNECTED))
                {
                    usb_printk("usb-hub: device connected to port %d\n", i);
                ...
                // and if a device was removed, delete it
                        ...
                        (hub->ports[i].flags & HUB_PORT_FLAG_CONNECTED))
                {
                    usb_printk("usb-hub: device removed from port %d\n", i);
...
{
    usb_printk("curstat %d, enabled %d, suspend %d, overcur %d, ", portst->curstat, portst->enabled, portst->suspend, portst->overcur);
    usb_printk("reset %d, res1 %d, power %d, lospeed %d, ", portst->reset, portst->res1, portst->power, portst->lospeed);
    usb_printk("hispeed %d, testmode %d, control %d, res2 %d\n", portst->hispeed, portst->testmode, portst->control, portst->res2);

    usb_printk("state_change %d, enabled_change %d, suspend_change %d, overcur_change %d, reset_change %d, res3 %d\n", portst->state_change, portst->enabled_change, portst->suspend_change, portst->overcur_change, portst->reset_change, portst->res3);
}
*/
...
int init_hub(struct usb_dev_t *usb)
{
    ...
    {
        usb_printk("usb-hub: insufficient memory to init USB hub\n");
        return -ENOMEM;
    }
    ...
    if(!endpoint)
    {
        usb_printk("usb-hub: USB hub has invalid interrupt endpoint\n");
        kfree(hub);
        return -EINVAL;
    }
    */
    ...
    {
        usb_printk("usb-hub: failed to get device descriptor\n");
        kfree(hub);
        return -EINVAL;
    }
    ...
    usb_printk("usb-hub: ports %d\n", hub->desc.ports);

    if(hub->desc.ports == 0)
    {
        usb_printk("usb-hub: USB hub has invalid port count\n");
        kfree(hub);
        return -EINVAL;
    }
    ...
    {
        usb_printk("usb-hub: insufficient memory to initialize USB hub ports\n");
        kfree(hub);
        return -ENOMEM;
    }
    ...
    for(i = 1; i <= hub->desc.ports; i++)
    {
        ...
        usb_printk("usb-hub: checking port %d (of %d)\n", i, hub->desc.ports);
        /*
    ...
    usb_printk("usb-hub: finished setting up USB hub\n");
